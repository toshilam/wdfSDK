...
(function(){
	...
	Tools.isLangCode = function(inLangCode)
	{
	...
	Tools.isPlatformCode = function(inValue)
	{
		var Config = wdf.Config;
		switch(inValue)
		{
			case Config.PF_CODE_WEB:
			case Config.PF_CODE_HTML5:	return true;
		}
		
		return false;
	}
	
	Tools.isGameType= function(inValue)
	{
		var Config = wdf.Config;
		switch(inValue)
		{
			case Config.GAME_TYPE_RNG_BJ_SP:
			case Config.GAME_TYPE_RNG_BAC_MP:
			case Config.GAME_TYPE_RNG_BAC_SP: 
			case Config.GAME_TYPE_RNG_BAC_MP_LOBBY: return true;
		}
	...
	Tools.loadNodeClasses = function(classes)
	{
		...
		for (var i = 0; i < numClass; i++) 
		{
			...
			wdf.Tracer.echo('Tools : loadNodeClasses : loading : ' + path);
	...
	{
		if(wdf.Tools.hasValue(inData) && Tools.isDateString(inData, inData.split(' ').length == 2))
		{
